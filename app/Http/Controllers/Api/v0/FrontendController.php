...
class FrontendController extends Controller
{
    ...
    public function savePlaylist(Request $request){
        ...
        $get_latest      = PlayListLog::where('video_id', $request->media_id)->latest()->first();
        $get_pre         = PlayListLog::where('video_id', $request->media_id)->latest()->skip(1)->first();
        try {
            if($get_latest && $get_pre){
                $up_logs         = PlayListLog::where('id', $get_latest ? $get_latest->id : null)
                    ->update([
                        'pre_time'          => $get_latest->last_watchtime,
                        'last_watchtime'    => $request->current_time,
                    ]);
                    return response()->json([
                        'success' => true,
                ...
                $playLog->video_id           = $request->media_id;
                $playLog->member_id          = auth()->user()->id;
                $playLog->ip                 = $this->getIp();
                $playLog->session_id         = Session::getId();
                $playLog->pre_time           = $get_pre ? $get_latest->last_watchtime : $request->current_time;
                $playLog->last_watchtime     = $request->current_time;
